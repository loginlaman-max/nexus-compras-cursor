...
export function CalcPrecosPageView() {
  ...
  return (
    <div className="nx-calc-page">
      <RelBanner icon={Calculator} title="Calculadora de Preços" subtitle="Defina o preço de venda a partir do custo, margem e carga tributária." />
      <div className="nx-calc-grid">
        <div className="card nx-calc-form">
          <div className="nx-calc-form-h">Parâmetros</div>
          <CalcField label="Custo do produto" prefix="R$" value={custo} onChange={setCusto} accent />
          <CalcField label="Margem desejada" suffix="%" value={margem} onChange={setMargem} hint="Margem líquida sobre o PV" />
          <div className="nx-calc-row2">
            <CalcField label="Despesas operacionais" suffix="%" value={desp} onChange={setDesp} />
            <CalcField label="Impostos / comissão" suffix="%" value={imp} onChange={setImp} />
          </div>
          <div className="nx-calc-carga">
            <span>Carga total sobre o PV</span>
            <strong className={valido ? "" : "is-bad"}>{tbPct(cargaPct)}</strong>
          </div>
          {!valido && <CalcWarn>A soma de margem + despesas + impostos passou de 100%.</CalcWarn>}
        </div>
        <div className="nx-calc-out">
          ...
          <div className="nx-calc-stats">
            <CalcStat label="Lucro líquido" value={tbBRL(lucro)} tone={lucro > 0 ? "ok" : "bad"} />
          ...
          <div className="card nx-calc-break">
            ...
            <CalcBar parts={[
              { label: "Custo", value: c, color: "hsl(var(--muted-foreground))" },
              { label: "Despesas", value: despVal, color: "hsl(var(--status-baixo))" },
              { label: "Impostos", value: impVal, color: "hsl(var(--status-excesso))" },
              { label: "Lucro", value: Math.max(0, lucro), color: "hsl(var(--status-ok))" },
            ]} total={pv} />
          </div>
        </div>
      </div>
